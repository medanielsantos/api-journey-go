...
import (
	"api-journey-go/internal/api/spec"
	"context"
	"errors"
	"net/http"
...
func (api API) PatchParticipantsParticipantIDConfirm(w http.ResponseWriter, r *http.Request, participantID string) *spec.Response {
	...
	if err != nil {
		if.errors.Is(err, pgstore.ErrNoRows) {
			return spec.PatchParticipantsParticipantIDConfirmJSON404Response(spec.Error{Message: "Participant not found"})
		}
		api.logger.Error("failed to get participant", zap.Error(err), zap.String("participant_id", participantID))

		return  spec.PatchParticipantsParticipantIDConfirmJSON500Response(spec.Error{Message: "Internal server error"})
	}
	...
	return spec.PatchParticipantsParticipantIDConfirmJSON204Response(nill)
}
